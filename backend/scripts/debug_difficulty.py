...
def check_snippet_difficulty():
    ...
    try:
        ...
        if snippet:
            ...
            print(
                f"  Processed Embedding: {'Yes' if snippet.processed_embedding else 'No'}"
            )
            print(
                f"  Normalized Features: {'Yes' if snippet.normalized_features else 'No'}"
            )
        else:
        ...
        with_diff = (
            session.query(Snippet).filter(Snippet.difficulty_score.isnot(None)).count()
        )
