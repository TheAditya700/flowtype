...
def get_agent_snapshots(limit: int = 100, db: Session = Depends(get_db)):
    """Get recent agent weight snapshots for monitoring learning progress."""
    snapshots = (
        db.query(AgentWeightSnapshot)
...
def get_agent_rewards(limit: int = 200, db: Session = Depends(get_db)):
    """Get recent reward history for reward decomposition analysis."""
    rewards = (
        db.query(AgentRewardHistory)
...
def get_agent_summary(db: Session = Depends(get_db)):
    ...
    avg_reward = (
        db.query(func.avg(AgentRewardHistory.reward))
        .order_by(desc(AgentRewardHistory.created_at))
        .limit(100)
        .scalar()
        or 0.0
    ...
    return {
        ...
        "current_precision_mean": (
            latest_snapshot.precision_mean if latest_snapshot else 0.0
        ),
