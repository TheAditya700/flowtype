...
def _calculate_stats(
    user_id: str, db: Session, user_obj: Optional[User] = None
) -> UserStats:
    ...
    def get_best_wpm(target_time: int):
        ...
        val = (
            db.query(func.max(TypingSession.actual_wpm))
            .filter(
                and_(
...
def get_leaderboard(
    mode: str = "60", exclude_anon: bool = False, db: Session = Depends(get_db)
):
    """
    ...
    if mode not in valid_modes:
        raise HTTPException(
            status_code=400, detail="Invalid mode; must be one of 15,30,60,120"
        )
    ...
    for u in users:
        ...
        entries.append(
            LeaderboardEntry(
                user_id=u.id, username=u.username, best_wpm=best, mode=mode
            )
        )
