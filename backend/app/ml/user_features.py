...
class UserFeatureExtractor:
    ...
    def __init__(self):
        ...
        self.burst_error_count = (
            0  # Count of errors that are part of a burst (>=2 consecutive)
        )
        ...
        self.roll_trans_counts = {"L2L": 0, "R2R": 0, "cross": 0}
        ...
        # --- F. LETTER CONFIDENCE ---
        self.char_stats = {
            c: {"presses": 0, "errors": 0} for c in string.ascii_lowercase
        }
    ...
    def compute_user_features(self) -> np.ndarray:
        ...
        kspc = safe_div(
            self.total_presses,
            (self.total_presses - self.backspace_count - self.total_errors),
            default=1.0,
        )

        backspace_ratio = safe_div(self.backspace_count, self.total_presses)
        burst_error_rate = safe_div(
            self.burst_error_count, self.total_errors
        )  # Fraction of errors that are bursts

        acc_L2L = 1.0 - safe_div(
            self.trans_stats["L2L"]["errors"], self.trans_stats["L2L"]["presses"]
        )
        acc_R2R = 1.0 - safe_div(
            self.trans_stats["R2R"]["errors"], self.trans_stats["R2R"]["presses"]
        )
        acc_cross = 1.0 - safe_div(
            self.trans_stats["cross"]["errors"], self.trans_stats["cross"]["presses"]
        )
        acc_repeat = 1.0 - safe_div(
            self.trans_stats["repeat"]["errors"], self.trans_stats["repeat"]["presses"]
        )
        ...
        wpm_effective = (
            np.mean(self.effective_wpm_history) if self.effective_wpm_history else 0.0
        )
        ...
        roll_L2L_rate = safe_div(
            self.roll_trans_counts["L2L"], self.trans_stats["L2L"]["presses"]
        )
        roll_R2R_rate = safe_div(
            self.roll_trans_counts["R2R"], self.trans_stats["R2R"]["presses"]
        )
        roll_cross_rate = safe_div(
            self.roll_trans_counts["cross"], self.trans_stats["cross"]["presses"]
        )
