...
def compute_difficulty_features(text: str) -> dict:
    ...
    avg_flow_segment_length = (
        sum(flow_segments_list) / len(flow_segments_list) if flow_segments_list else 1.0
    )
    ...
    for i in range(n - 1):
        if chars[i] == chars[i + 1]:
            ...
            if current_run > max_char_run:
                max_char_run = current_run
            if current_run == 2:
                double_letter_count += 1
            elif current_run == 3:
                triple_letter_count += 1
            elif current_run > 3:
                triple_letter_count += 1  # Count 4+ as triple+ for simplicity?
            current_run = 1
