      ...
      <div className="w-full max-w-7xl mx-auto p-6">
        <div className="text-center py-20">
          ...
          <p className="text-gray-500">
            Start typing to build your profile, then come back.
          </p>
    ...
    <div className="w-full max-w-[1600px] mx-auto p-6 flex flex-col gap-6">
      ...
      <div className="flex items-center gap-4 pb-2 border-b border-gray-800">
        ...
        <div>
          ...
          <p className="text-gray-400">
            Track speed, consistency, and activity over time
          </p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        {[15, 30, 60, 120].map((mode, idx) => {
          const bestWpm = Math.round(
            (summary[
              `best_wpm_${mode === 15 ? "15" : mode}` as keyof typeof summary
            ] as number) ?? 0,
          );
          const colors = [
            "text-emerald-400",
          ...
          return (
            <div
              key={mode}
              className="bg-gray-900 p-6 rounded-xl border border-gray-800 relative overflow-hidden group"
            >
              <div className="absolute top-0 right-0 p-4 opacity-10 group-hover:opacity-20 transition-opacity">
                <Award size={64} className="text-white" />
              </div>
              <div className="text-gray-500 text-sm font-mono mb-2 uppercase tracking-wider">
                {mode} Seconds Best
              </div>
              <div
                className={`text-5xl font-bold font-mono ${colors[idx % colors.length]}`}
              >
                {bestWpm}
              </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div className="bg-gray-900 p-6 rounded-xl border border-gray-800 flex flex-col justify-between">
          ...
          <div className="text-4xl font-bold text-emerald-400 font-mono">
            {summary.avg_accuracy.toFixed(0)}
        ...
        <div className="bg-gray-900 p-6 rounded-xl border border-gray-800 flex flex-col justify-between">
          ...
          <div className="text-4xl font-bold text-yellow-400 font-mono">
            {formatMinutes(summary.total_time_typing)}
            <span className="text-xl text-yellow-600 ml-1">min</span>
          </div>
        </div>
      </div>
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-gray-500 font-mono">
                Toggle between speed and accuracy
              </p>
            </div>
          </div>
            ...
            <ResponsiveContainer>
              ...
              >
                ...
                <YAxis
                  ...
                  stroke="#374151"
                  domain={
                    chartMode === "wpm" ? ["dataMin - 5", "auto"] : [0, 100]
                  }
                  tickMargin={10}
                  tickFormatter={
                    chartMode === "wpm" ? undefined : (v) => `${v}%`
                  }
                />
                <Tooltip
                  ...
                  formatter={(value: number, name) => {
                    if (chartMode === "accuracy") {
                      const label =
                        name === "ema_accuracy" ? "Accuracy EMA" : "Accuracy";
                      return [`${Math.round(value as number)}%`, label];
                    }
                    return [
                      Math.round(value as number),
                      name === "ema_wpm" ? "Trend (EMA)" : "WPM",
                    ];
                  }}
                  contentStyle={{
          ...
          <div className="flex items-center justify-between mb-6">
            ...
            <div className="flex gap-3 text-xs font-mono">
              <div className="px-3 py-1 bg-gray-800 rounded-md border border-gray-700 text-gray-400">
                Streak:{" "}
                <span className="text-white font-bold">{current_streak}</span>
              </div>
              <div className="px-3 py-1 bg-gray-800 rounded-md border border-gray-700 text-gray-400">
                Best:{" "}
                <span className="text-white font-bold">{longest_streak}</span>
              </div>
            </div>
          </div>
            ...
            <div className="scale-x-100 origin-left">
              <CalendarHeatmap
                ...
                tooltipDataAttrs={(value: any) => ({
                  title:
                    value && value.date
                      ? `${value.date}: ${value.count} session(s)`
                      : "No activity",
                })}
                transformDayElement={(
