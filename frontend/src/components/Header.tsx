...
const Header: React.FC<HeaderProps> = ({ isPaused, sessionStarted }) => {
  ...
  return (
    <header
      className={`w-full flex items-center justify-between mb-4 z-20 relative h-16 transition-all duration-500 ${isVisible ? "opacity-100 translate-y-0" : "opacity-0 -translate-y-4 pointer-events-none"}`}
    >
      {/* Left: Logo */}
      ...
      <div className="flex items-center gap-3 justify-end ml-auto">
        {(isTypePage || isLeaderboardPage) && (
          <div
            className={`flex items-center gap-2 bg-container px-3 py-2 rounded-lg transition-all duration-500 h-12 ${isVisible ? "opacity-100 translate-y-0" : "opacity-0 -translate-y-4 pointer-events-none"}`}
          >
