...
const ReplayChunkStrip: React.FC<ReplayChunkStripProps> = ({ events }) => {
  ...
  const needsGhostSnippet =
    visibleSnippetIndices.length === 1 && totalSnippets > 1;
  const ghostEvents = needsGhostSnippet
    ? visibleEvents.filter(
        (e) => (e.snippetIndex ?? 0) === visibleSnippetIndices[0],
      )
    : [];
      ...
      <div className="flex justify-between items-center mb-6">
        ...
        <div className="flex items-center gap-4">
          <span className="text-xs text-gray-500 font-mono">
            Snippets {startSnippetIdx + 1}-
            {Math.min(endSnippetIdx, totalSnippets)} / {totalSnippets}
          </span>
      ...
      {mode === "rollover" ? (
        <div className="flex flex-wrap items-end content-start gap-y-6 min-w-full">
          {visibleEvents.map((m, i) => {
            ...
            const prevSameSnippet =
              i > 0 &&
              (visibleEvents[i - 1].snippetIndex ?? 0) === (m.snippetIndex ?? 0)
                ? visibleEvents[i - 1]
                : null;
            const possible = Boolean(
              prevSameSnippet &&
              getHand(prevSameSnippet.char) &&
              getHand(m.char),
            );
            const isRollover = possible && m.isRollover;

            const prevSnippetIndex =
              i > 0 ? visibleEvents[i - 1].snippetIndex : m.snippetIndex;
            const isNewSnippet = m.snippetIndex !== prevSnippetIndex && i !== 0;
            const isLastInSnippet =
              i === visibleEvents.length - 1 ||
              (i < visibleEvents.length - 1 &&
                visibleEvents[i + 1].snippetIndex !== m.snippetIndex);

            return (
              <React.Fragment key={`rollover-fragment-${i}`}>
                {isNewSnippet && (
                  <div
                    key={`roll-snippet-break-${i}`}
                    className="w-full h-px bg-gray-700 my-4"
                  />
                )}
                <div
                  ...
                  <div
                    className={`w-2 rounded-t-sm mb-1 transition-all ${
                      isRollover
                        ...
                        : possible
                          ? "border border-purple-300/60 bg-purple-900/10"
                ...
                {isLastInSnippet && (
                  ...
                  >
                    <div
                      ...
                      style={{ height: "50px" }}
                    />
                    <span className="font-mono text-xl leading-none px-[2px] min-w-[1ch]">
                      \u00A0
                    </span>
                  </div>
                )}
          ...
          {needsGhostSnippet && (
            <React.Fragment>
              ...
              {ghostEvents.map((m, i) => {
                ...
                const possible = Boolean(
                  prevSameSnippet &&
                  (prevSameSnippet.snippetIndex ?? 0) ===
                    (m.snippetIndex ?? 0) &&
                  getHand(prevSameSnippet.char) &&
                  getHand(m.char),
                );
        ...
        {mode === "rollover" ? (
          <>
            <div className="flex items-center gap-1">
              <div className="w-3 h-3 border border-purple-300/60 rounded-sm bg-purple-900/20"></div>{" "}
              Possible
            </div>
            <div className="flex items-center gap-1">
              <div className="w-3 h-3 bg-purple-500 rounded-sm"></div> Actual
              rollovers
            </div>
          </>
        ) : (
          <>
            <div className="flex items-center gap-1">
              <div className="w-3 h-3 bg-green-500 rounded-sm"></div> &lt; 150ms
            </div>
            <div className="flex items-center gap-1">
              <div className="w-3 h-3 bg-yellow-500 rounded-sm"></div> 150-300ms
            </div>
            <div className="flex items-center gap-1">
              <div className="w-3 h-3 bg-red-500 rounded-sm"></div> &gt; 300ms
            </div>
          </>
        )}
      </div>
