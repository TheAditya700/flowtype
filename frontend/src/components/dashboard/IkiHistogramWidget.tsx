...
const IkiHistogramWidget: React.FC<IkiHistogramWidgetProps> = ({
  replayEvents,
}) => {
  ...
  const filteredEvents = useMemo(() => {
    ...
    if (filter === "left")
      return replayEvents.filter((e) => getHand(e.char) === "L" && e.iki > 0);
    if (filter === "right")
      return replayEvents.filter((e) => getHand(e.char) === "R" && e.iki > 0);
      ...
      <div className="text-xs text-gray-500 flex justify-between border-t border-gray-700 pt-2">
        ...
        <span>
          ...
          {filteredEvents.length > 0
            ? Math.round(
                filteredEvents.reduce((s, e) => s + e.iki, 0) /
