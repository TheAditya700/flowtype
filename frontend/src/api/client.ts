...
export async function fetchNextSnippet(
  userState: UserState,
  currentSnippetId?: string,
): Promise<SnippetRetrieveResponse | null> {
...
export async function fetchUserStatsDetail(
  userId: string,
): Promise<UserStatsDetail> {
  return callApi<UserStatsDetail>(`/users/${userId}/stats/detail`, "GET");
}

export async function fetchLeaderboard(
  mode: "15" | "30" | "60" | "120" = "60",
  excludeAnon: boolean = false,
): Promise<LeaderboardEntry[]> {
  return callApi<LeaderboardEntry[]>(
    `/users/leaderboard?mode=${mode}&exclude_anon=${excludeAnon}`,
    "GET",
  );
}
