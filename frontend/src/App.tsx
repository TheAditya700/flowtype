...
function App() {
  ...
  const [sessionResultData, setSessionResultData] =
  ...
  const [liveStats, setLiveStats] = useState({
    ...
    sessionMode: "free" as "15" | "30" | "60" | "120" | "free",
  ...
  const fetchMoreSnippets = async (
    ...
    try {
      ...
      for (let i = 0; i < count; i++) {
        const currentRecentIds = (overrideRecentIds || recentSnippetIds).filter(
          Boolean,
        ); // Filter falsy values
        ...
        const currentId =
          snippetQueue.length > 0 ? snippetQueue[0]?.id : undefined;
  ...
  const handleSnippetComplete = async (stats: {
    // Made async
    ...
    const log: SnippetLog = {
      ...
      started_at: new Date(
        stats.keystrokeEvents[0]?.timestamp || Date.now(),
      ).toISOString(),
      completed_at: new Date(
        stats.keystrokeEvents[stats.keystrokeEvents.length - 1]?.timestamp ||
          Date.now(),
      ).toISOString(),
    ...
    const correctKeystrokes = updatedAllKeystrokes.filter(
      (k) => k.isCorrect && !k.isBackspace,
    ).length;
    const totalErrors = updatedAllKeystrokes.filter(
      (k) => !k.isCorrect && !k.isBackspace,
    ).length;

    // Calculate duration from keystroke timestamps
    const sortedKeystrokes = [...updatedAllKeystrokes].sort(
      (a, b) => a.timestamp - b.timestamp,
    );
    const duration_ms =
      sortedKeystrokes[sortedKeystrokes.length - 1].timestamp -
      sortedKeystrokes[0].timestamp;
    const duration_seconds = duration_ms / 1000;

    const currentSessionWpm =
      duration_seconds > 0
        ? correctKeystrokes / 5 / (duration_seconds / 60)
        : 0;
    const totalKeystrokes = updatedAllKeystrokes.filter(
      (k) => !k.isBackspace,
    ).length;
    const currentSessionRawWpm =
      duration_seconds > 0 ? totalKeystrokes / 5 / (duration_seconds / 60) : 0;
    ...
    const newText = sessionSummary.text
      ? sessionSummary.text + " " + snippetText
      : snippetText;
    const newTotalWords =
      sessionSummary.totalWords + currentSnippet.words.length;
    ...
    const updatedRecentIds = [...recentSnippetIds, currentSnippet.id]
      .filter(Boolean)
      .slice(-20);
  ...
  const handlePause = async () => {
    ...
    const keystrokesToSave =
      pendingKeystrokes.current.length > 0
        ? pendingKeystrokes.current
        : allKeystrokes;

    if (keystrokesToSave.length === 0) return; // No keystrokes to save

    const sortedKeystrokes = [...keystrokesToSave].sort(
      (a, b) => a.timestamp - b.timestamp,
    );
    const duration_ms =
      sortedKeystrokes[sortedKeystrokes.length - 1].timestamp -
    ...
    return (
      <div className="fixed inset-0 bg-bg text-text flex items-center justify-center p-6 font-mono">
        <div className="w-full max-w-md rounded-xl px-8 py-10 flex flex-col items-center gap-4 text-center bg-bg/90 backdrop-blur">
          ...
          <div className="text-subtle text-sm pb-2">
    ...
    <div className="flex-grow flex flex-col w-full max-w-[1800px] mx-auto relative font-mono">
          ...
          <div className="flex flex-col items-center w-full">
            ...
            <div className="w-full mb-8 h-16 relative">
              <div className="absolute bottom-0 left-0 w-full flex justify-between items-end transition-all duration-500">
                <h1
                  className={`text-3xl font-bold text-subtle tracking-tighter ${
                    !liveStats.sessionStarted
            ...
            <div className="w-full transition-all duration-500 scale-100">
              {snippetQueue.length > 0 ? (
                ...
                <div className="text-center text-subtle animate-pulse">
                  loading...
                </div>
              )}
            </div>
      ...
      {!sessionResultData && (
        <div className="absolute bottom-4 left-0 right-0 text-center text-subtle text-sm transition-opacity duration-300 opacity-100">
          <span className="bg-container px-2 py-1 rounded text-xs mr-2 text-text">
            enter
          </span>{" "}
          to pause
        </div>
